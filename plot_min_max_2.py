...
def draw_delay(max_delay, algorithms, keys, save_dir):
    plt.figure()
    plt.xlabel("Budget")
    plt.ylabel("Max Queuing Delay (ms)")
    plt.grid(linestyle='--')
    plt.tight_layout()
...
def draw():
    ...
    dir_path = "result/surrogate/other_algorithms/{}u-10s".format(users)
    ...
    dir_path = "result/surrogate/{}u-10s".format(users)
    algorithms_equal_weight = ['min_max_surrogate_relaxation']

    file_list = get_json_file_list(dir_path)
    max_delay_ew, running_time_ew = read_data(file_list, keys, algorithms_equal_weight)

    max_delay.update(max_delay_ew)
    running_time.update(running_time_ew)
    algorithms += algorithms_equal_weight
    ...
    save_dir = "result/surrogate/figure/{}u-10s".format(users)
